...
def Udisg(Udis, lam, q0, q1, q2, q3):
    ...
    th1 = np.arccos((lam+np.cos(2*pi*k/n)) /
                    np.sqrt((lam+np.cos(2*pi*k/n))**2+np.sin(2*pi*k/n)**2))
    B(Udis, q0, q1, th1)
    Udis.barrier()
...
def plot_Mag_of_ground_state(vexact, mag_sim):
    ...
    plt.savefig(
        "./images/1D_n=4_Ising_spin_chain/Magnetization_of_the_ground_state_of_n=4_Ising_spin_chain.png")
    plt.show()
    plt.clf()
...
def plot_Time_evolution_all_up_state(vexact_t, magt_sim):
    ...
    plt.savefig(
        "./images/1D_n=4_Ising_spin_chain/Time_evolution_all_up_state.png")
    plt.show()
    plt.clf()
